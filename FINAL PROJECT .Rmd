...
OBA18_19 <- OBA18_19[c("Dec. Long.", "Dec. Lat.", "MonthJul", "Associated plant", "Species")]
...
PLOTTING!! :)
```{r}
#plotting all the finalized data 
complete_data %>% 
  group_by(City, MonthJul, MinTemp, MaxTemp) %>% 
  summarize(Number_Bees = n_distinct(Species)) %>% 
  ggplot() %>% 
  geom_point(aes(x = MinTemp, y = Number_Bees, colour = "hotpink", size = 3, shape = ))
```
